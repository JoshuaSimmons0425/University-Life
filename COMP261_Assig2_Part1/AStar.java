...
public class AStar {
    ...
    public static List<Edge> findShortestPath(Stop start, Stop goal) {
        ...
        fringe.add(new SearchQueueItem(start, null, 0, start.distanceTo(goal))); // Adding the start node to the fringe with heuristic cost (h(n))
        ...
        while(!fringe.isEmpty()){
            ...
            for(Edge edge: currentStop.getEdgesOut()){
                ...
                double newCost = costSoFar.get(currentStop) + edge.distance(); // g(n)
                ...
                if(!costSoFar.containsKey(neighbour) || newCost < costSoFar.get(neighbour)){
                    ...
                    double priority = newCost + neighbour.distanceTo(goal); // Total estimated cost: f(n) = g(n) + h(n)
                    fringe.add(new SearchQueueItem(neighbour, edge, newCost, priority)); // Add neighbour to fringe for further searching
