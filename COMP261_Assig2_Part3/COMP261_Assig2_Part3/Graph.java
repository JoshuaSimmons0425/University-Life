    ...
    private void createAndConnectEdges() {
        for (Line line : lines) {
            ...
            for (int i = 0; i < line.getStops().size() - 1; i++) {
                ...
                Edge reverseEdge = new Edge(to, from, transpType, line, distance);
                edges.add(edge);
                edges.add(reverseEdge);
                from.addTransportNeighbour(to);
                to.addTransportNeighbour(from);
            }
        }
    }
        ...
        for (Stop s1: stops) {
            for (Stop s2: stops) {
                ...
                if (distance <= walkingDistance) {
                    ...
                    s1.addWalkingNeighbour(s2);
                    s2.addWalkingNeighbour(s1);
